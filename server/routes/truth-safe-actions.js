...
function item(id, name, location, bytes) {
  ...
  return { id, name, location, sizeMB, reclaimable: sizeMB === null ? 'UNAVAILABLE' : `${sizeMB} MB`, measured: sizeMB !== null, selected: sizeMB !== null && sizeMB > 0 };
}
...
router.post('/cleanup-plan', async (_req, res) => {
  ...
  try {
    ...
    const total = observed.reduce((sum, x) => sum + (x.sizeMB || 0), 0);
...
router.post('/kill-port', async (req, res) => {
  ...
  try {
    ...
    const killedPids = stdout.trim() ? JSON.parse(stdout) : [];
    const pids = Array.isArray(killedPids) ? killedPids : [killedPids];
...
router.post('/toggle-startup', async (req, res) => {
  ...
  try {
    const script = `$name=$args[0]; $backup='HKCU:\\Software\\WinSuite\\DisabledStartupItems'; $run='HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Run'; if($null -eq (Get-ItemProperty -Path $run -Name $name -ErrorAction SilentlyContinue) -and $enable -eq $true){ $v=Get-ItemProperty -Path $backup -Name $name -ErrorAction SilentlyContinue; if($null -eq $v){ throw 'Startup item is not present in the current-user Run key and no WinSuite backup exists.' }; New-ItemProperty -Path $run -Name $name -Value $v.$name -PropertyType String -Force | Out-Null; Remove-ItemProperty -Path $backup -Name $name -ErrorAction SilentlyContinue } elseif($enable -eq $false){ $v=(Get-ItemProperty -Path $run -Name $name -ErrorAction SilentlyContinue).$name; if($null -eq $v){ throw 'Startup item is not present in the current-user Run key.' }; New-Item -Path $backup -Force | Out-Null; New-ItemProperty -Path $backup -Name $name -Value $v -PropertyType String -Force | Out-Null; Remove-ItemProperty -Path $run -Name $name -ErrorAction Stop } else { throw 'Startup item state is already the requested state or is not managed by the current-user Run key.' }; 'OK'`;
    ...
    if (!/OK/i.test(stdout)) throw new Error('Startup state change was not confirmed by PowerShell.');
