...
router.post('/cleanup-plan', async (_req,res)=>{
  ...
  try{const measured=await Promise.all(candidates.map(async([id,name,location])=>item(id,name,location,await measureTree(location))));const observed=measured.filter(x=>x.measured);const total=observed.reduce((s,x)=>s+(x.sizeMB||0),0);res.json({platform:isMac?'macos':'windows',planItems:measured,totalReclaimableMB:observed.length?total:null,totalReclaimableGB:observed.length?+(total/1024).toFixed(1):null,measurement:observed.length?'observed':'unavailable',summary:observed.length?'Cleanup candidates are based on measured filesystem contents and have real execution bindings.':'No executable cleanup candidate could be measured on this host.'});}catch(err){res.status(500).json({error:err?.message||'Cleanup measurement failed.'});}
});

router.post('/kill-port',async(req,res)=>{
  const port=Number(req.body?.port); if(!Number.isInteger(port)||port<1||port>65535)return res.status(400).json({error:'A valid TCP port (1-65535) is required.'});
  ...
  try{const script='$p=[int]$args[0]; $c=Get-NetTCPConnection -LocalPort $p -ErrorAction SilentlyContinue; $ids=@($c | Select-Object -ExpandProperty OwningProcess -Unique); foreach($id in $ids){if($id -and $id -ne 0){Stop-Process -Id $id -Force -ErrorAction Stop}}; $ids | ConvertTo-Json -Compress';const {stdout}=await execFileAsync('powershell.exe',['-NoProfile','-NonInteractive','-Command',script,'--',String(port)],{encoding:'utf8',timeout:10000,windowsHide:true});const parsed=stdout.trim()?JSON.parse(stdout):[];const pids=Array.isArray(parsed)?parsed:[parsed];res.json({success:true,killedPids:pids.filter(v=>Number.isInteger(v)),measurement:'observed'});}catch(err){res.status(500).json({success:false,error:err?.message||'Port termination failed.'});}
});
...
router.post('/toggle-startup',async(req,res)=>{
  ...
  try{const desired=enable?'$true':'$false';const script=`$name=$args[0];$enable=${desired};$backup='HKCU:\\Software\\WinSuite\\DisabledStartupItems';$run='HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Run';if($enable){$v=Get-ItemProperty -Path $backup -Name $name -ErrorAction SilentlyContinue;if($null -eq $v){throw 'No WinSuite backup exists for this startup item.'};New-ItemProperty -Path $run -Name $name -Value $v.$name -PropertyType String -Force|Out-Null;Remove-ItemProperty -Path $backup -Name $name -ErrorAction SilentlyContinue}else{$v=(Get-ItemProperty -Path $run -Name $name -ErrorAction SilentlyContinue).$name;if($null -eq $v){throw 'Startup item is not present in the current-user Run key.'};New-Item -Path $backup -Force|Out-Null;New-ItemProperty -Path $backup -Name $name -Value $v -PropertyType String -Force|Out-Null;Remove-ItemProperty -Path $run -Name $name -ErrorAction Stop};'OK'`;const {stdout}=await execFileAsync('powershell.exe',['-NoProfile','-NonInteractive','-Command',script,'--',itemName],{encoding:'utf8',timeout:10000,windowsHide:true});if(!/OK/i.test(stdout))throw new Error('Startup state change was not confirmed.');res.json({success:true,itemName,enabled:enable,measurement:'observed'});}catch(err){res.status(500).json({success:false,error:err?.message||'Startup change failed.',measurement:'unavailable'});}
});
