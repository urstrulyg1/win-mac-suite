...
import { HardDrive, Trash2, Folder, FileCode, CheckCircle2, ChevronRight } from 'lucide-react';
    ...
    <div className="relative z-10 max-w-[1600px] mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        <div>
          <div className="inline-flex items-center gap-2 mb-2">
            ...
            <span className="pill" style={{ backgroundColor: 'var(--color-surface-2)', color: 'var(--color-ink-3)', borderColor: 'var(--color-line)' }}>
              Live Storage Analytics · Click Any Row To Inspect
            </span>
          </div>
      ...
      <div className="flex items-center gap-1.5 p-1 rounded-2xl border overflow-x-auto" style={{ backgroundColor: 'var(--color-surface-2)', borderColor: 'var(--color-line)' }}>
        {[
          ...
          { id: 'developer' as const, label: isMac ? 'Developer Cleanup (Xcode/Pods/Brew)' : 'Developer Caches (VS/NPM)', icon: FileCode },
              ...
              {(storageData?.largeFiles || []).map((lf: any, idx: number) => (
                <button
                  ...
                  onClick={() =>
                    setInspectItem({
                      title: lf.name,
                      category: 'Large Storage File',
                      badge: lf.size,
                      subtitle: 'Large file candidate located in user directories.',
                      details: [
                        { label: 'File Name', value: lf.name },
                        { label: 'Full Absolute Path', value: lf.path, isCode: true },
                        { label: 'Allocated File Size', value: lf.size },
                      ],
                    })
                  }
